...
export const Route = createFileRoute("/pair")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tell us what you're eating and get the best wines from the list to match." },
    ],
  }),
  component: PairPage,
});
...
function PairPage() {
  ...
  async function pair() {
    if (!food.trim()) { toast.message("Tell us what you're eating first."); return; }
    if (!imageDataUrl && !wineListText.trim()) {
      toast.message("Add a wine list — photo or pasted text."); return;
    }
      ...
      <main className="mx-auto max-w-5xl px-6 py-12 md:px-10 md:py-16">
        <header className="mb-10 animate-reveal">
          ...
          <p className="mt-3 max-w-xl text-muted-foreground">
            Describe the dish — we'll match it against the list with the right structure, acidity, and weight.
          </p>
        </header>
        ...
        <section className="grid gap-8 md:grid-cols-2">
          <div className="animate-reveal space-y-5">
            <label className="block">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gold/80">The dish</span>
            ...
            <label className="block">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gold/80">Wine list (photo)</span>
              <div
            ...
            <label className="block">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gold/80">Or paste the list</span>
              <textarea
                value={wineListText}
                onChange={(e) => setWineListText(e.target.value)}
                placeholder={"Pinot Noir, Domaine Drouhin 2019 — $78\nBarolo, Vietti 2018 — $145\n…"}
                rows={6}
                maxLength={4000}
